...
{
    sfEvent event;
    while (sfRenderWindow_pollEvent(window_params->window, &event)) {
        event.type == sfEvtClosed ? sfRenderWindow_close(window_params->window)
            : 0;
        event.type == sfEvtResized ? resize_event(event, window_params) : 0;
    }
}
